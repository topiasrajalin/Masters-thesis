import pandas as pd
from config import (
    COPENHAGEN_MONTHLY_PRICES_EUR, COPENHAGEN_MONTHLY_RETURNS,
    COPENHAGEN_WEEKLY_PRICES, COPENHAGEN_WEEKLY_RETURNS
)
...
def calculate_monthly_returns():
    prices_df = pd.read_excel(COPENHAGEN_MONTHLY_PRICES_EUR)
    return calculate_returns(prices_df, COPENHAGEN_MONTHLY_RETURNS)


def calculate_weekly_returns():
    prices_df = pd.read_excel(COPENHAGEN_WEEKLY_PRICES)
    ...
    returns_df.to_excel(COPENHAGEN_WEEKLY_RETURNS, index=False)
    return returns_df
